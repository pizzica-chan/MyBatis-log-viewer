...
**外れ方は 2 通りあり、画面への出方が違います。**

- **正規表現は当たったが日時を読めない行**は、読み飛ばしとして件数と例を出します。
  直す先は日時書式です
- **正規表現がそもそも当たらない行**は、継続行（スタックトレース）と見分けられないので
  読み飛ばしには数えません。正規表現が丸ごと外れていると **SQL が 0 件**になるので、
  そのときは画面に手がかりを出します

どちらも「この行で試す」にログの 1 行を貼れば理由が出ます。
...
- 書式は自動判定の候補にも入ります。判定は**一致した行数**で決め、同数のときだけ
  組み込みが選ばれます。つまり**組み込みより多くの行に当たる広い書式を登録すると、
  組み込み書式のログを読むときもそちらが選ばれます**。選ばれたときは書式欄に
  「自動判定（名前・利用者定義）」と出るので、意図しないときは書式を明示指定するか、
  その書式を消してください
