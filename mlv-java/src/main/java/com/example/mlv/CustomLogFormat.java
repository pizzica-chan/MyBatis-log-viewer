...
/**
 * 利用者が {@code mlv-log-formats.txt} に定義したログ書式（{@link LogFormatStore}）。
 * 1 行を 1 本の正規表現で解析する。
 *
 * <p>組み込み書式（{@link LogFormat}）はバイト列のまま数バイトを見て弾けるが、この書式は
 * 行を UTF-8 デコードしてから正規表現にかけるため重い。100 万行（25 万 SQL ブロック）の
 * 索引構築で 1,852ms → 3,836ms（Windows 11 / JDK 8 / -Xmx2g、交互に 7 回ずつの中央値。
 * 同じ内容を書式違いで書いた別ファイル同士の比較）。<strong>この重さを払うのは、この書式を
 * 選んだ取り込みだけ</strong>で、組み込み書式の経路は 1 命令も増えない
 ...
 *
 * <p>レベルは組み込み書式と違って語の検査をせず、取り出した値をそのまま採る
 * （{@code NOTICE} でも {@code 警告} でも通る）。ただし絞り込みの比較を揃えるため、
 * 組み込み書式と同じく ASCII の範囲で大文字に直す（{@code info} → {@code INFO}）。
 *
 * <h2>暴走する正規表現への備え</h2>
...
public final class CustomLogFormat {
    ...
    /** メッセージの必須グループ名。ここから MyBatis のブロックを探す。 */
    ...
    CustomLogFormat(String id, String name, String patternText, String timestampPattern) {
        ...
        if (!groups.contains(GROUP_MESSAGE)) {
            throw new IllegalArgumentException(
                    "pattern に名前付きグループ (?<" + GROUP_MESSAGE + ">…) が必要です"
                    + "（MyBatis の Preparing / Parameters / Total はここから探します）");
        }
     ...
     * 呼び出し側の扱いが変わる。前者は直前のエントリの続き、後者は<strong>直すべき
    ...
    Map<String, String> matchedGroups(final String line) {
        return guarded(new Supplier<Map<String, String>>() {
            ...
            public Map<String, String> get() {
                ...
                values.put(GROUP_MESSAGE, group(m, true, GROUP_MESSAGE));
                return values;
            }
        });
    }
