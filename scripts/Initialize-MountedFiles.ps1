...
.DESCRIPTION
    docker-compose.yml は mlv-saved-searches.json と mlv-log-formats.txt を
...
function Initialize-SavedSearchesFile {
    ...
    New-EmptyFile `
        -Path (Join-Path $RepoRoot "mlv-saved-searches.json") `
...
function Initialize-LogFormatsFile {
    ...
    New-EmptyFile `
        -Path (Join-Path $RepoRoot "mlv-log-formats.txt") `
        -Body "# mlv の利用者定義ログ書式（画面の「書式の管理」から編集できます）`r`n"
}
